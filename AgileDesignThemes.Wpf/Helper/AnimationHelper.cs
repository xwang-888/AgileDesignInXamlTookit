...
using AgileDesignThemes.Wpf.Extension;
...
namespace AgileDesignThemes.Wpf.Helper
{
    public class AnimationHelper
    {
        ...
        internal static void DecomposeGeometryStr(string geometryStr, out double[] arr)
        {
            ...
            for (var i = 0; i < collection.Count; i++) arr[i] = collection[i].Value.Value<double>();
        }
        ...
        internal static Geometry ComposeGeometry(string[] strings, double[] arr)
        {
            ...
            for (var i = 0; i < arr.Length; i++)
            {
                var s = strings[i + 1];
                var n = arr[i];
                if (!double.IsNaN(n)) builder.Append(n).Append(s);
            }
